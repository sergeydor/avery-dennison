...
{
    BOOL res;
    res = HidD_GetInputReport(device->device_handle, data, length);
    if (!res) {
        register_error(device, "HidD_GetInputReport");
        return -1;
    }
    return 0; 
}
...
{
    BOOL res;
    res = HidD_SetOutputReport(device->device_handle, data, length);
    if (!res) {
        register_error(device, "HidD_SetOutputReport");
        return -1;
    }
    return 0;
}
